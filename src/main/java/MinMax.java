...
public class MinMax {
    ...
    private Nodes initNodes;			//node for the initial state

    private ArrayList<Nodes> stateList;	//a list to contain all possible moves for X
    ...
    MinMax(String[] init)
    {
        ...
        stateList = createStateList("max", initNodes);		//get list of all X moves from initial state
...
//        System.out.println("\nAt the end of FindMoves, the states and min/max values are:\n");
    ...
    {
        initNodes = new Nodes(initState, 0);
    }
     ...
     */
    private ArrayList<Nodes> createStateList(String minOrMax, Nodes stateNodes)
    {

        ArrayList<Nodes> stateList = new ArrayList<Nodes>();		//array list to hold all possible moves

        String[] initState = stateNodes.getInitStateString();	//string of initial state of board
        ...
        if(minOrMax == "max")		//list of all X moves
        {
            ...
            for(int x = 0 ; x < initState.length ; x++)
            {
                if(initState[x].equals("b"))								//its a blank space
                {
                    ...
                    Nodes temp = new Nodes(copyInit, x+1);				//make a Node out of it
     ...
     * @param list
     */
    void printList(ArrayList<Nodes> list)
    {
        for(int x = 0; x < list.size(); x++ )		//get a node from the list
        {
            Nodes temp = list.get(x);
     ...
     * @param x_or_y
     */
    void setStateList_MinMaxValues(ArrayList<Nodes> theStateList, int x_or_y)
    {
        if(x_or_y == 1)						//set the minMax values for X
        {
            for(int x =0; x < theStateList.size(); x++)
            {
                Nodes temp = theStateList.get(x);
     ...
     * @param list
     */
    void print_minMax(ArrayList<Nodes> list)
    {
    ...
    int Min(Nodes state)
    {
        ...
        ArrayList<Nodes> listOfStates = createStateList("min", state);
        ...
        {
            Nodes temp = listOfStates.get(y);					//get node off list
    ...
    int Max(Nodes state)
    {
        ...
        //create list of possible moves
        ArrayList<Nodes> listOfStates = createStateList("max", state);
